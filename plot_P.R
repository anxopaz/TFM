...
AA<-matrix(rep(0,11*5),nrow=11,ncol=5)

names<-c("MSY","Bmsy","K","n","Bly.Bmsy")
colnames(AA)<-names
...
colnames(spict_estimations_MAPE)[3]<-("Bly.Bmsy")
colnames(spict_estimations_RMSE)[3]<-("Bly.Bmsy")
colnames(aspic_estimations_MAPE)[3]<-("Bly.Bmsy")
colnames(aspic_estimations_RMSE)[3]<-("Bly.Bmsy")
...
BB<-matrix(rep(0,11*5),nrow=11,ncol=5)
names<-c("MSY","Bmsy","K","n","Bly.Bmsy")
colnames(BB)<-names
rownames(BB)<-value

colnames(spict_estimations)[9]<-("Bly.Bmsy")
colnames(spict_estimations)[4]<-("K")
colnames(spict_estimations_MAPE)[3]<-("Bly.Bmsy")
colnames(spict_estimations_RMSE)[3]<-("Bly.Bmsy")
colnames(aspic_estimations_MAPE)[3]<-("Bly.Bmsy")
colnames(aspic_estimations_RMSE)[3]<-("Bly.Bmsy")
...
CC<-matrix(rep(0,11*5),nrow=11,ncol=5)
names<-c("MSY","Bmsy","K","n","Bly.Bmsy")
colnames(CC)<-names
rownames(CC)<-value
...
colnames(plotting)[5]<-"BT/Bmsy"
names[5]<-"BT/Bmsy"

clase<-c(rep("0.3",length(value)),
         rep("0.5",length(value)),
         rep("0.7",length(value)))
plotting<-cbind(plotting,clase)

clase2<-c(rep("0.3",length(values2)),
          rep("0.5",length(values2)),
          rep("0.7",length(values2)))
...
plo<-list()
for (i in names){
  inde<-which(plotting[,6]=="real"|plotting[,6]=="aspic_mean"|plotting[,6]=="spict_mean")
  medias<-as.numeric(plotting[inde,i])
  inde2<-which(plotting[,6]=="real"|plotting[,6]=="aspic_lq"|plotting[,6]=="spict_lq")
  ...
  lq[ceros]=0
  inde3<-which(plotting[,6]=="real"|plotting[,6]=="aspic_uq"|plotting[,6]=="spict_uq")
  uq<-as.numeric(plotting[inde3,i])
  cvs<-factor(c(rep(0.3,3),rep(0.5,3),rep(0.7,3)))
  valores<-rep(c("real","ASPIC","SPiCT"),3)
  ceros<-which(valores=="real")
  uq[ceros]=0
...
plo2<-list()
for (i in names){
  inde<-which(plotting[,6]=="aspic_RMSE"|plotting[,6]=="spict_RMSE")
  RMSE<-as.numeric(plotting[inde,i])
  inde2<-which(plotting[,6]=="aspic_MAPE"|plotting[,6]=="spict_MAPE")
  MAPE<-as.numeric(plotting[inde2,i])
  cvs<-factor(c(rep(0.3,2),rep(0.5,2),rep(0.7,2)))
...
jpeg("proceso1.jpeg", width = 2000, height = 1500, res = 300)
...
jpeg("proceso2.jpeg", width = 2000, height = 1500, res = 300)
...
jpeg("proceso3.jpeg", width = 2000, height = 1500, res = 300)
