...
F_SPM <- C/ (( B_SPM+c(B_SPM[-1],B_SPM[length(B_SPM)]))/2)
...
CV_N=0.3
C2<-matrix(rep(0,nrep*length(years)),nrow=nrep)
for(i in 1:nrep){
  capturas<-Sampling_Catch(Pop.Mod, type="catch weight",CV_CN=CV_N)
  C2[i,]<-capturas[["weight"]][,,1]
  cat(100*i/nrep,"%\n")
}
...
plot(years,B_SPM,type="l",lwd=2,col="red",
     ylab="Biomasa",xlab="Ano",main="Traxectoria da Biomasa SPM")

plot(years,C,type="l",lwd=2,col="red",ylim=c(0,max(c(C,C2))),
     ylab="Capturas",xlab="Ano",main="Traxectoria das Capturas")
lines(years,C2,lwd=2,col="green")
legend(years[0.75*length(years)],0.95*max(c(C,C2)),c("Capturas reais","Capturas CV=0.1"),
...
plot(years,IA_SPM_0,type="l",lwd=2,col="red",
     ...
     ylab="Índice de abundancia",xlab="Tempo",main="Índice de abundancia")
for(i in 1:5){
  index<-IA_SPM[i,]
...
for(irep in 1:nrep){
  ...
  inp<-list(timeC=years, obsC=C2[irep,], obsI=(IA_SPM[irep,]), timeI=years,
            ini=list(q=q),
            aspic=list(shape="GENFIT",conditioning="YLD",objfn="SSE"))
  waspic(inp,file=paste0(irep,".a7inp"))
  faspic(paste0(irep,".a7inp"),t=36)
  ...
  while (tt==FALSE){
    ...
    IA_new<-Abundance_Index(B=B_SPM,nrep=1,years=years,CV=CV,q=q)
    cnew<-Sampling_Catch(Pop.Mod, type="catch weight",CV_CN=CV_N)
    cnew<-capturas[["weight"]][,,1]
    inp<-list(timeC=years, obsC=cnew, 
  ...
  while (errorcode!=0) {
    ...
    IA_new<-Abundance_Index(B=B,nrep=1,years=years,CV=CV,q=q)
    cnew<-Sampling_Catch(Pop.Mod, type="catch weight",CV_CN=CV_N)
    cnew<-capturas[["weight"]][,,1]
    inp<-list(timeC=years, obsC=cnew, obsI=(IA_new), timeI=years,
              ini=list(q=q),
              aspic=list(shape="GENFIT",conditioning="YLD",objfn="SSE"))
    waspic(inp,file=paste0(irep,".a7inp"))
    faspic(paste0(irep,".a7inp"),t=36)
...
for(i in 1:nrep){
  ...
  inp1=list(timeC=years, obsC=C2[i,], obsI=IA_SPM[i,], timeI=years)
  res<- fit.spict(inp1)
  ...
  while (convergence!=0) {
    ...
    cnew<-Sampling_Catch(Pop.Mod, type="catch weight",CV_CN=CV_N)
    cnew<-capturas[["weight"]][,,1]
    inp1=list(timeC=years, obsC=cnew, obsI=IA_new, timeI=years)
    res<- fit.spict(inp1)
    convergence<-res$opt$convergence
  }
