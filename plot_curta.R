...
DD<-matrix(rep(0,11*6),nrow=11,ncol=6)
colnames(DD)<-names
rownames(DD)<-value
...
for (i in names) {
  DD[1,i]<-SPM_values[,i]
  a_iestimation<-aspic_estimations[,i]
  DD[2,i]<-mean(a_iestimation)
  qq<-quantile(a_iestimation,probs=c(0.05,0.95))
  DD[3,i]<-qq[[1]]
  DD[4,i]<-qq[[2]]
  s_iestimation<-spict_estimations[,i]
  DD[5,i]<-mean(s_iestimation)
  qq<-quantile(s_iestimation,probs=c(0.05,0.95))
  DD[6,i]<-qq[[1]]
  DD[7,i]<-qq[[2]]
  DD[8,i]<-aspic_estimations_RMSE[,i]
  DD[9,i]<-aspic_estimations_MAPE[,i]
  DD[10,i]<-spict_estimations_RMSE[,i]
  DD[11,i]<-spict_estimations_MAPE[,i]
}

DD<-cbind(DD,value)


DD2<-matrix(rep(0,2*12),ncol=2)
colnames(DD2)<-names2
rownames(DD2)<-values2

for (i in names2) {
  DD2[1,i]<-aspic_RMSE["media",i]
  DD2[2,i]<-aspic_RMSE["lq",i]
  DD2[3,i]<-aspic_RMSE["uq",i]
  DD2[4,i]<-aspic_MAPE["media",i]
  DD2[5,i]<-aspic_MAPE["lq",i]
  DD2[6,i]<-aspic_MAPE["uq",i]
  DD2[7,i]<-spict_RMSE["media",i]
  DD2[8,i]<-spict_RMSE["lq",i]
  DD2[9,i]<-spict_RMSE["uq",i]
  DD2[10,i]<-spict_MAPE["media",i]
  DD2[11,i]<-spict_MAPE["lq",i]
  DD2[12,i]<-spict_MAPE["uq",i]
}
################################################################################
################################################################################
################################################################################

plotting<-rbind(AA,BB,CC,DD)
plotting2<-rbind(AA2,BB2,CC2,DD2)
colnames(plotting)[6]<-"BT/Bmsy"
names[6]<-"BT/Bmsy"

clase<-c(rep("20-",length(value)),
         rep("30-",length(value)),
         rep("40-",length(value)),
         rep("50-",length(value)))
plotting<-cbind(plotting,clase)

clase2<-c(rep("20-",length(values2)),
          rep("30-",length(values2)),
          rep("40-",length(values2)),
          rep("50-",length(values2)))
value2<-rep(values2,4)
...
for (i in names){
  ...
  valores<-rep(c("real","ASPIC","SPiCT"),4)
  ...
  cvs<-factor(c(rep("2020-",3),rep("2030-",3),rep("2040-",3),rep("2050-",3)))
  valores<-rep(c("real","ASPIC","SPiCT"),4)
  ceros<-which(valores=="real")
  uq[ceros]=0
...
for (i in names){
  ...
  cvs<-factor(c(rep("20-",2),rep("30-",2),rep("40-",2),rep("50-",2)))
  valores<-rep(c("ASPIC","SPiCT"),4)
...
for (i in names2){
  ...
  cvs<-factor(c(rep("2020-",2),rep("2030-",2),rep("2040-",2),rep("2050-",2)))
  valores<-rep(c("ASPIC","SPiCT"),4)
...
for(i in 1:6) {
  ...
  geom_errorbar(data=plo[[i]][-c(1,4,7,10),],aes(x=cvs,group=valores, color=valores,
                                                 ymin=lq, ymax=uq), 
                inherit.aes = FALSE,
...
for(i in c(5,6)) { 
  plot_list[[i]]<-plot_list[[i]] + labs(x="Período")
}

jpeg("curtas1.jpeg", width = 2000, height = 1500, res = 300)
...
for(i in c(5,6)) { 
  plot_list2[[i]]<-plot_list2[[i]] + labs(x="Período")
  plot_list3[[i]]<-plot_list3[[i]] + labs(x="Período")
}
...
jpeg("curtas2.jpeg", width = 2000, height = 1500, res = 300)
...
for(i in 2) { 
  plot_list4[[i]]<-plot_list4[[i]] + labs(x="Período")
  plot_list5[[i]]<-plot_list5[[i]] + labs(x="Período")
}
...
jpeg("curtas3.jpeg", width = 2000, height = 1500, res = 300)
